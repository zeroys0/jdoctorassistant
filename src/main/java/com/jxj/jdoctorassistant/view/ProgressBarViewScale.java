 ...
 * 自定义绚丽的ProgressBar. 
 */  
public class ProgressBarViewScale extends View {  
    ...
    public ProgressBarViewScale(Context context) {  
        super(context);  
    ...
    public ProgressBarViewScale(Context context, AttributeSet attrs) {  
        super(context, attrs); 
        this.context=context;
        init();  
    }  
    ...
    public ProgressBarViewScale(Context context, AttributeSet attrs, int defStyleAttr) {  
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {  
        ...
        if (width == 0 || height == 0) {  
            ...
            ARC_LINE_WIDTH = ARC_LINE_LENGTH / 15;  
    ...
    protected void onDraw(Canvas canvas) {  
        ...
        //circleRadius=circleRadius*4/5;
        while (drawDegree <= ARC_FULL_DEGREE) {  
            double a = (start + drawDegree) / 180 * Math.PI;  
            float lineStartX = centerX - circleRadius*3/4 * (float) Math.sin(a);  
            float lineStartY = centerY + circleRadius*3/4 * (float) Math.cos(a);  
        ...
        canvas.drawCircle(centerX, centerY, (circleRadius - ARC_LINE_LENGTH) * 1.0f, textBgPaint);  
        ...
        String text = (int) (100 * (100-progress) / max) + "";  
        ...
        text = context.getString(R.string.progressbar_text);
    ...
    public void setProgress(final int progress) {  
        new Thread(new Runnable() {  
            @Override  
            public void run() {  
                float oldProgress = ProgressBarViewScale.this.progress;  
                for (int i = 1; i <= 100; i++) {  
                    ProgressBarViewScale.this.progress = oldProgress + (progress - oldProgress) * (1.0f * i / 100);  
                    postInvalidate();  
